...
export const translations = {
  ko: {
    ...
    privacy_s6_body: "앱 내 마이페이지 → 계정 삭제 버튼으로 언제든 탈퇴할 수 있습니다. 탈퇴 시 개인정보·GPS 경로·사진은 삭제되고 다시 로그인할 수 없으며, 레이스 정합성을 위해 일부 기록은 익명 처리되어 보관됩니다.",
  ...
  en: {
    ...
    privacy_s6_body: "You can delete your account at any time via My Page → Delete Account. Your personal info, GPS routes, and photos are deleted and you can no longer log in; some records are retained in anonymized form to keep races consistent.",
  ...
  es: {
    ...
    privacy_s6_body: "Puedes eliminar tu cuenta cuando quieras desde Perfil → Eliminar cuenta. Tu información personal, rutas GPS y fotos se eliminan y ya no podrás iniciar sesión; algunos registros se conservan de forma anonimizada para mantener la coherencia de las carreras.",
  ...
  ja: {
    ...
    privacy_s6_body: "マイページ → アカウント削除からいつでも退会できます。退会時に個人情報・GPS経路・写真は削除され、再ログインできなくなります。レースの整合性のため、一部の記録は匿名化して保管されます。",
  ...
  zh: {
    ...
    privacy_s6_body: "可随时通过“我的”→删除账号进行注销。注销时个人信息、GPS 轨迹和照片将被删除，且无法再次登录；为保持比赛一致性，部分记录将以匿名形式保留。",
