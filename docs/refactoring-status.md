...
### 코덱스 교차 검토 — 인증 재배치 적대적 검증

인증 순서 변경은 위험도가 높아 별도로 적대적 검토를 받았다(다섯 가설을 각각 반증 시도).
**재배치로 도입된 우회 경로는 없다**는 결론이다.

- 수용 토큰 집합: Firebase 정상 시 전후 동일. 미초기화 시에만 "서명된 자체 JWT"가 새로 통과
- `alg=none`은 `parseSignedClaims`가, RSA↔HMAC 혼동은 `verifyWith(SecretKey)`의 키 타입이 차단
- 보호 API의 폴백 실패는 전부 401(초기화 검사 직후 앱이 삭제되는 경쟁 조건 포함)
- `AuthContext`는 `finally`가 `Error`까지 커버해 누수 없음

> **남은 위험은 재배치가 아니라 JWT 시크릿의 엔트로피다.** `Keys.hmacShaKeyFor`는 32바이트
> 미만만 막고 낮은 엔트로피 키는 통과시킨다. 시크릿이 추측되면 `fuid`를 임의로 넣은 토큰을
> 만들 수 있고 그 값은 관리자 판정에도 쓰인다. 재배치는 이 공격이 Firebase 장애 중에도
> 통하게 만들어 영향 범위를 넓힌다 — 운영 `JWT_SECRET`의 무작위성·재사용 여부를 확인할 것.

### 리뷰가 잡은 테스트 공백 둘

- **`JwtServiceTest`의 만료 테스트가 공허했다.** `verify`를 호출만 하고 아무것도 단정하지
  않았고 주석까지 "어느 쪽이든 예외 없이 동작하면 통과"라고 적혀 있어, **만료 검사가 통째로
  사라져도 통과**하는 상태였다. 경계(`expiryDays=0`) 대신 확실히 지난 시각(−1일)을 쓰고 같은
  시크릿으로 검증해 empty의 원인이 서명 불일치가 아님을 보장하도록 고쳤다. 반대 방향(만료
  전은 present)도 함께 넣어 "항상 empty라서 통과"가 아님을 보인다.
- **`recordSelfOnce`에 테스트가 없었다** — 추가할 때 안 붙였다. 기록·억제·창 방향·판정 범위
  4케이스를 넣었다.

> **교훈: 새 코드에 테스트를 붙였는지, 붙였다면 그 테스트가 실제로 뒤집히는지 두 번 확인할 것.**
> 이번 6차에서 공허한 단정(JwtService)과 테스트 자체의 부재(recordSelfOnce)가 각각 하나씩
> 나왔다. 경품 테스트에서 쓴 방법(로직을 일부러 뒤집어 몇 개가 깨지는지 보기)이 유효하다.

### 주석이 실제 보장보다 강했던 것 둘 (정정 완료)

- `recordSelfOnce`는 **순차 반복만** 막는다. 존재 확인과 저장이 한 문장이 아니고 유니크 제약도
  없어 동시 요청 N개는 각각 기록된다. 완전 차단은 시간 버킷 컬럼 + 부분 유니크 인덱스(스키마
  변경)나 엔드포인트 요청 제한이 필요하다 — 현재는 보존 정책이 증가량의 상한 역할을 한다.
- 스냅샷 자리 선점의 **"60초 침묵 = 닫힌 탭"은 부정확**하다. 브라우저가 백그라운드 탭 타이머를
  분 단위로 늦추므로 살아 있는 탭도 침묵한다. 그러면 새 런이 자리를 가져가고 원래 탭은 이후
  저장이 거부된다(라이브 기록은 계속되나 새로고침 후 복구 불가). 화면에 떠 있는 쪽이 자리를
  갖는 편이 낫다고 보고 받아들였다.

### 여전히 열려 있는 것

- **`JWT_SECRET` 엔트로피 확인** — 위 참조. 코드로 확인할 수 없는 운영 값이다.
