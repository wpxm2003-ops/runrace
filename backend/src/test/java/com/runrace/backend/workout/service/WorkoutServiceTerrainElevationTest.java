...
class WorkoutServiceTerrainElevationTest {
  ...
  void GPS_고도를_좌표_기준_지형고로_교체한다() {
    ...
    List<WorkoutService.PathPoint> result = service.withTerrainElevation(path);
  ...
  void 소스가_비활성이면_GPS_고도를_그대로_둔다() {
    ...
    assertThat(service.withTerrainElevation(path))
  ...
  void 빈_경로는_그대로_돌려준다() {
    WorkoutService service = serviceWith(fakeTerrain());
    assertThat(service.withTerrainElevation(List.of())).isEmpty();
  }
}
